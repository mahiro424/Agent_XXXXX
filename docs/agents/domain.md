...
当输出中提及某个领域概念时（无论是在 issue 标题、重构提案、假设还是测试名称中），使用 `CONTEXT.md` 中定义的术语。不要偏离到术语表中明确避免使用的同义词上。

如果需要的概念尚未出现在术语表中，这本身就是一个信号——要么正在创造项目不使用的语言（应重新考虑），要么确实存在一个缺口（记录下来，供 `/grill-with-docs` 使用）。

## 标记 ADR 冲突

如果输出与现有 ADR 存在冲突，应明确指出，而不是静默覆盖：
